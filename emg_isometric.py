...
parser.add_argument("--thresh", type=float, default=20.0,
                    help="Contraction detection threshold as %% of envelope max (default 20).")
...
def detect_contractions(env, thresh_pct, merge_gap_s, strong_min_s, weak_min_s):
    """Threshold-based detection. Returns (strong, weak) lists of (start_s, end_s)."""
    valid = ~np.isnan(env)
    vmax = np.nanmax(env)
    thresh = thresh_pct / 100.0 * vmax
...
if args.contractions:
    ...
    strong, weak = detect_contractions(
        envelopes[:, plot_indices[0]], args.thresh, args.merge_gap,
        args.strong_min, args.weak_min)
...
# ===== FIGURE 2: overlay normalized envelopes =====
NORM_PTS = 101  # 0..100 %


def normalized_envelopes(env, contractions):
    """Resample each contraction envelope to 0-100 % of duration, amplitude as % of group max."""
    curves = []
    ...
    if not curves:
        return np.array(curves), 1.0
    vmax = max(c.max() for c in curves)
    return np.array(curves), vmax
...
for ax, contractions, cmap, name in [
        (axes2[0], strong, cmap_strong, "Strong"),
        (axes2[1], weak, cmap_weak, "Weak")]:
    curves, vmax = normalized_envelopes(envelopes[:, plot_indices[0]], contractions)
    for k, c in enumerate(curves):
        ax.plot(np.linspace(0, 100, NORM_PTS), 100.0 * c / vmax,
                color=cmap[k], linewidth=1.2, label=f"{name} {k+1}")
    if len(curves):
        mean_curve = 100.0 * np.mean(curves, axis=0) / vmax
        ax.plot(np.linspace(0, 100, NORM_PTS), mean_curve, "k-", linewidth=2.5, label="Mean")
    ax.set_ylabel("Envelope (% of group max)")
    ax.set_title(f"{name} contractions (n={len(curves)})")
    ax.grid(True, alpha=0.3)
    ax.legend(loc="lower right", fontsize=8)
axes2[1].set_xlabel("Contraction duration (%)")
fig2.suptitle(f"Overlaid contraction envelopes (Ch{args.channels[0]}) - "
